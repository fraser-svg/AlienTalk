//! Stage 2: Structural minification — JSON minification and list collapsing.
...
pub fn stage_minify(text: &str, intensity: f64) -> String {
    if intensity < 0.1 {
...
mod tests {
    ...
    fn minify_json() {
        ...
        let result = stage_minify(input, 1.0);
        assert_eq!(result, r#"{"name":"John","age":30,"city":"New York"}"#);
    }
    ...
    fn zero_intensity_skips() {
        let input = r#"{"a": 1}"#;
        let result = stage_minify(input, 0.0);
