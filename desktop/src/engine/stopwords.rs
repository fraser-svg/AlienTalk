//! Stage 2: Stop-Word Stripping — conservative set with protected words.
...
/// Compiled stop-word regex: matches words in STOP_WORDS that are NOT in PROTECTED_WORDS.
...
pub fn stage_stopwords(text: &str, intensity: f64) -> String {
    if intensity < 0.1 {
...
mod tests {
    ...
    fn strips_stop_words() {
        let result = stage_stopwords("The analysis of the data was performed with the tools", 1.0);
        assert_eq!(result, "analysis data performed tools");
    }

    #[test]
    fn preserves_protected() {
        let result = stage_stopwords("If the value is not valid then return an error", 1.0);
    ...
    fn zero_intensity_skips() {
        let result = stage_stopwords("The quick brown fox", 0.0);
